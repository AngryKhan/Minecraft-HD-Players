...
import java.io.*;
import net.minecraft.src.getSkinUrl;
import net.minecraft.src.getUrlB;
...
public abstract class EntityPlayer extends EntityLiving
{
	...
	private getSkinUrl getskinurl;
	public boolean hasSkinThread;
	public boolean checkedForHDSkin;
	public String HDSkinUrl;
    ...
    public EntityPlayer(World par1World)
    {
		...
		if(mod_HDSkins.mode.equals("thread")){
		hasSkinThread = false;
		
		skinUrl = (new StringBuilder()).append("https://s3.amazonaws.com/HDMinecraftSkins/").append(username).append(".png").toString();
		HDSkinUrl = skinUrl;
		checkedForHDSkin = false;
		getSkinUrl gsUrl = new getSkinUrl(this);
		}
    }
	...
	if(mod_HDSkins.mode.equals("thread")){
		if(checkedForHDSkin == false){
			skinUrl = (new StringBuilder()).append("https://s3.amazonaws.com/HDMinecraftSkins/").append(username).append(".png").toString();
		}
        
	}
	if(mod_HDSkins.mode.equals("classic")){
		skinUrl = getUrlB.getUrlB(this);
	
		
	}
	if(mod_HDSkins.capes == true){
		cloakUrl = (new StringBuilder()).append("http://www.hdminecraftskins.com/MinecraftCloaks/").append(username).append(".png").toString();
	}else{
		cloakUrl = (new StringBuilder()).append("http://s3.amazonaws.com/MinecraftCloaks/").append(username).append(".png").toString();
	}
    }
